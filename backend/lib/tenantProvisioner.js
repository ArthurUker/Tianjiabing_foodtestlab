...
// 学校代码规范：小写字母、数字、连字符；1~40 字符。用于 schema 名安全。
...
export async function provisionSchool({
  ...
  // ④ 租户内首个 admin（幂等：已存在则跳过）
  let adminCreated = false
  const hash = await bcryptjs.hash(pw, 10)
  await prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe(`SET LOCAL search_path TO "${schema}", public`)
    const found = await tx.$queryRawUnsafe(
      `SELECT 1 FROM "User" WHERE "username" = 'admin' LIMIT 1`
    )
    if (found.length) {
      log(`ℹ️ 租户 ${code} 已存在 admin，跳过创建`)
      return
    }
    await tx.$executeRawUnsafe(
      `INSERT INTO "User"
         ("id","username","password_hash","full_name","role","status","school_code","created_at","updated_at")
       VALUES ($1,'admin',$2,'Administrator','admin','active',$3,now(),now())`,
      `u_${code}_admin`,
      hash,
      code
    )
    adminCreated = true
    log(`✅ 已为租户 ${code} 创建 admin 账号`)
  })

  return { code, schema, created, adminCreated }
}
