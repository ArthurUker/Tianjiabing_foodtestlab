...
export const CASE_DEFS = [
  ...
  // ⚠️ 本组为**人工浏览器验证用例**（超管控制台逐条执行、人工判定并留证），
  //    不是自动化测试。开放接口的自动化覆盖在 backend/tests/openapi/（contract.test.mjs 纯函数、
  //    db-readonly-checks.mjs 只读数据库行为），两者互补，不可互相代替。
  { group: 'open_api_permission', groupName: '开放接口验证（人工浏览器用例）', cases: [
    { id: 'OA-G1', title: 'OA-G1 超管开通对接方并生成密钥（明文仅显示一次）',
      guide: '目的：确认超管可新建对接方并生成 API Key，且明文密钥只在生成时显示一次。\n步骤：1.平台超管登录 admin-schools.html → 左侧「开放接口」。2.点「新建对接方」→ 填名称（如 测试对接方）、IP 白名单留空、限流默认 60 → 创建。3.在该对接方「凭证管理」页点「生成新密钥」→ 弹窗出现完整密钥（oap_ 开头）。4.关闭弹窗后重新进入「凭证管理」，确认只能看到前缀+后四位，无法再看到明文。\n判定：对接方创建成功 + 密钥仅显示一次 + 列表仅展示前缀/后四位 → 通过；明文可反复查看或创建失败 → 失败截图。' },
    { id: 'OA-G2', title: 'OA-G2 未授权学校一律 403（越权防护）',
      guide: '目的：确认第三方只能读取被授权的学校，猜测其他 school_code 无效。\n步骤：1.超管为对接方只勾选 A 校（如 tjb）并保存。2.用该对接方的密钥调 GET /api/open/v1/test-records?school_code=<A校> → 正常返回。3.改 school_code=<未授权的 B 校> → 应返回 403（SCHOOL_NOT_AUTHORIZED）。4.调 GET /api/open/v1/sync/manifest?school_code=<B校> → 同样 403。5.调 GET /api/open/v1/schools → 仅返回 A 校。\n判定：授权校可读 + 未授权校 403 + schools 列表只含授权校 → 通过；任一未授权校可读到数据 → 失败截图（越权，严重）。' },
    { id: 'OA-G3', title: 'OA-G3 未开放检测类型不可读（病原体默认不开放）',
      guide: '目的：确认类型白名单生效，病原体需显式开启。\n步骤：1.超管为对接方勾选 A 校的「餐具洁净度/果蔬农残」，**不勾选病原体** → 保存。2.调 GET /api/open/v1/test-records?school_code=<A校>&test_type=pathogen → 应 403（TYPE_NOT_AUTHORIZED）。3.不带 test_type 拉取 → 返回条目中不含 pathogen 记录。4.超管再勾选病原体并保存 → 重复第 2 步 → 应能读到病原体记录。\n判定：未开放类型 403 且不出现在结果中 + 显式开启后可读 → 通过；未开启即可读到病原体 → 失败截图（严重）。' },
    { id: 'OA-G4', title: 'OA-G4 字段脱敏：检测人姓名不下发（含嵌套）',
      guide: '目的：确认对外 JSON 已剔除内部字段与人名类 PII（含复检记录、修改轨迹内的姓名）。\n步骤：1.超管为对接方配置 A 校授权，「检测人」选择「不下发」并保存。2.调 GET /api/open/v1/test-records?school_code=<A校>&limit=50，用文本搜索返回 JSON：不应出现 inspector、recheckRecords[].user、modificationLogs 等键，也不应出现检测人姓名（如「李丹」）。3.超管把「检测人」改为「下发」并保存，重新拉取 → 应出现 inspector 字段。4.在「数据预览」页抽样核对 JSON 形态。\n判定：不下发时无任何人名类字段、下发时 inspector 正常出现、且 modificationLogs 等内部字段始终不出现 → 通过；出现内部字段或未授权的人名 → 失败截图（严重）。' },
    { id: 'OA-G5', title: 'OA-G5 增量拉取：游标分页不重不漏、跨校游标被拒',
      guide: '目的：确认增量同步协议正确。\n步骤：1.调 GET /api/open/v1/test-records?school_code=<A校>&limit=10 → 记下返回的 next_cursor 与 count。2.带该 cursor 再请求 → 返回条目不应与上一页重复（按 record_code 核对）；连续翻页直至 has_more=false。3.统计各页条数之和，与 GET /api/open/v1/sync/manifest?school_code=<A校> 的 total 对比（明细条数应等于 total；若不同请记录差异原因）。4.把 A 校的 cursor 用于 B 校请求 → 应 400（CURSOR_SCHOOL_MISMATCH）。5.乱造 cursor（如 abc）→ 应 400（INVALID_CURSOR）。\n判定：翻页不重不漏 + 条数与清单一致 + 跨校/非法游标被拒 → 通过；出现重复或丢单、跨校游标可用 → 失败截图。' },
    { id: 'OA-G6', title: 'OA-G6 授权变更触发重新同步（409 + 补拉历史）',
      guide: '目的：确认范围变更后游标失效、扩容可补拉历史数据。\n步骤：1.对接方 A 校仅开「餐具」→ 用密钥拉一轮并持有 cursor。2.超管新增开放「食用油」并保存。3.用旧 cursor 继续请求 → 应返回 409（SCOPE_CHANGED）。4.改调 sync/manifest?detail=1 → total 应包含新增类型的记录（说明历史数据可补拉）。5.不带 cursor 重新拉取（或在对方侧按清单重新对账）→ 能拿到食用油记录。\n判定：旧游标 409 + 扩容后清单/明细包含新类型历史数据 → 通过；旧游标仍返回旧范围数据（未感知变更）→ 失败截图。' },
    { id: 'OA-G7', title: 'OA-G7 删除与清单对账（删除可被感知）',
      guide: '目的：确认记录删除后第三方可通过清单比对感知（平台为硬删除，无回收站）。\n步骤：1.调 sync/manifest?school_code=<A校> 记录 total 与 digest。2.用学校 manager 在该校删除一条检测记录（或新增一条后再删除）。3.再次调 sync/manifest（先不带 detail）→ total/digest 应变化。4.调 detail=1 → 清单中不再包含被删的 record_code。5.若期间无任何变更，连续两次调用 detail=1 → digest 应完全一致。\n判定：删除后 total/digest 变化且清单缺失该记录 + 无变更时 digest 稳定 → 通过；删除后清单无变化（对方永远感知不到删除）→ 失败截图（严重）。' },
    { id: 'OA-G8', title: 'OA-G8 密钥吊销/停用对接方/限流生效',
      guide: '目的：确认凭证与对接方状态的即时管控能力。\n步骤：1.持有效密钥调 /api/open/v1/ping → 200。2.超管在「凭证管理」吊销该密钥 → 同一密钥再调 ping → 应 401（CREDENTIAL_REVOKED）。3.重新生成密钥，确认 200；超管「停用对接方」→ 再调 → 应 403（CLIENT_DISABLED）；恢复启用 → 调通。4.连续快速请求超过限流阈值（默认 60 次/分钟，可临时把限流调为 5 便于测试）→ 应返回 429 且带 Retry-After 头。5.不带密钥请求 → 401（MISSING_KEY）。\n判定：吊销/停用即时生效 + 超限 429 + 无密钥 401 → 通过；吊销后仍可调用 → 失败截图（严重）。' },
    { id: 'OA-G9', title: 'OA-G9 超管配置接口权限隔离（非平台超管 403）',
      guide: '目的：确认开放接口的配置能力仅平台超管可用。\n步骤：1.用学校 manager 账号的 JWT 调 GET /api/admin/open-api/clients → 应 403。2.用学校 admin（若有）同样调用 → 应 403（平台超管 = role=admin 且无 schoolCode）。3.未携带任何凭证调用对外接口 GET /api/open/v1/profile → 应 401。4.平台超管调用 GET /api/admin/open-api/clients → 200。\n判定：学校侧角色 403 + 未认证 401 + 平台超管 200 → 通过；任一越权返回数据 → 失败截图（严重）。' },
    { id: 'OA-G10', title: 'OA-G10 配置导出与导入（灾后重建）',
      guide: '目的：确认配置可导出留档，并能导入重建（public 段不参与备份恢复，故依赖此功能）。\n步骤：1.超管在「开放接口」页点「导出配置」→ 下载 JSON，确认含 clients/credentials（仅 key_hash，无明文密钥）/grants。2.将该 JSON 备份留存。3.测试环境或经批准后：删除一个对接方（或直接「导入配置」同一份 JSON）→ 点「导入配置」粘贴 JSON → 导入。4.核对：对接方、授权范围、密钥哈希均恢复，且原密钥仍可正常调用（ping 200）。5.再导入一次同一份 JSON → 不应重复创建凭证（按 key_hash 去重）。\n判定：导出内容完整且无明文密钥 + 导入后配置与密钥可用 + 重复导入不产生重复凭证 → 通过；导出含明文密钥或导入后原 Key 失效 → 失败截图。' },
    { id: 'OA-G11', title: 'OA-G11 字段字典：可读、与真实响应同源、标注自定义字段',
      guide: '目的：确认对接方无需靠"猜"就能知道字段含义。\n步骤：1.超管进「开放接口」→ 选中对接方 → 「接入说明」→ 第 2 块「字段字典」，选学校与类型 → 点「加载字段字典」。2.核对表头六列（路径/中文名/类型/单位/必现/可空/说明）均有值；`record_code`、`test_date`、`result.rluValue` 等条目存在。3.切到另一类型，确认字段随之变化。4.若该校配了自定义字段，确认其以琥珀色底 +「自定义」标记、类型显示 unknown。5.用同一学校/类型的真实拉取结果（「数据预览」）比对：字典里 `required=是` 的字段在每条记录中都应出现。\n判定：字典可加载、字段随类型变化、自定义字段有标注、required 标注与真实数据一致 → 通过；字典为空/字段明显缺失 → 失败截图。' },
    { id: 'OA-G12', title: 'OA-G12 合成样例：非真实数据、覆盖多场景、不泄露姓名',
      guide: '目的：确认对方在没有任何真实数据时也能开发。\n步骤：1.「接入说明」第 3 块「合成样例」，选学校与类型 → 点「加载样例」。2.核对返回项 `synthetic:true`、`item.record_code` 以 `SAMPLE-` 开头。3.餐具应能看到 pass / fail / recheck_passed / sparse 四类场景；果蔬/肉蛋/食用油**不应**出现复检场景。4.在被授权学校**尚无任何检测记录**的情况下重复第 1 步，应仍能拿到样例。5.若「检测人姓名」为不下发，确认样例中无 `inspector` 字段、也无任何示例姓名文本。\n判定：样例可加载、场景齐备且不越界、无真实数据与姓名 → 通过；出现真实记录内容或姓名 → 失败截图（严重）。' },
    { id: 'OA-G13', title: 'OA-G13 接入包下载：内容完整且不含密钥',
      guide: '目的：确认可一次性交付对方所需材料，且不泄露密钥。\n步骤：1.「接入说明」第 4 块点「下载接入包（Markdown）」。2.打开文件核对包含：接口说明与端点表、**已保存**的开放范围表、字段字典、合成样例 JSON、错误码表、同步规则、接入检查清单。3.全文搜索 `oap_`、`key_hash`、`sha256` 等：除说明性文字外**不应出现任何真实密钥或哈希**。4.若此时在「学校授权」有未保存修改，确认页面有黄色提示且接入包内容仍为已保存版本。\n判定：内容七要素齐全 + 无密钥/哈希 + 未保存修改不影响导出 → 通过；含密钥或授权摘要与已保存不一致 → 失败截图（严重）。' },
    { id: 'OA-G14', title: 'OA-G14 字段撤回：关闭检测人姓名后不再下发（且可恢复）',
      guide: '目的：确认"撤回的字段不再出现"，并确认对方能据此清除本地旧值。\n步骤：1.为对接方配置某校授权并**开启**「下发检测人姓名」→ 保存；用「数据预览」确认记录含 `inspector`。2.回到「学校授权」把该校改为**不下发** → 保存（会提示需通知对方重新同步）。3.再次「数据预览」：`inspector` 字段应完全消失；用密钥调 `GET /profile` 确认 `projection_fingerprint` 已变化、`scope_version` 已递增；调 `GET /sync/manifest` 确认 `digest` 与调用前不同（即使没有任何记录变化）。4.对应的 `OA-G1` 密钥调 `GET /test-records?...`（带旧游标）应返回 409。5.再改回「下发」并保存 → 预览与接口恢复含 `inspector`。\n判定：关闭后接口与预览均无姓名 + 指纹/版本/digest 变化 + 旧游标 409 + 可恢复 → 通过；关闭后仍能看到姓名 → 失败截图（严重）。' },
    { id: 'OA-G15', title: 'OA-G15 统计口径可解释：scope_total / included / excluded 自洽',
      guide: '目的：确认合格率差异能被解释，而不是"以某个接口为准"。\n步骤：1.用密钥调 `GET /stats?school_code=<校>`，记录 `scope_total`、`included_total`、`excluded[]`、`pass_rate_detail`。2.调 `GET /sync/manifest?school_code=<校>`，核对 `total` 与 `scope_total` 的关系（若该校存在检测日期缺失的记录，两者相差的条数应正好等于 `excluded` 中 `missing_or_invalid_test_date` 的 count）。3.核对 `pass_rate_detail.numerator/denominator` 与 `pass_count/scope_total` 一致；`denominator` 为 0 时应返回 `null` 而不是 0。\n判定：三项数字自洽且排除原因可追溯 → 通过；出现无法解释的条数差 → 失败截图并附两个接口的原始返回。' },
    { id: 'OA-G16', title: 'OA-G16 游标绑定：换筛选条件/旧版本游标被明确拒绝',
      guide: '目的：确认客户端不能靠"换条件复用游标"扩大或错配数据。\n步骤：1.用密钥调 `GET /test-records?school_code=<校>&test_type=<类型A>&limit=5`，取回 `next_cursor`。2.用同一游标请求**类型B**（同校）→ 应 `400 CURSOR_FILTER_MISMATCH`。3.用同一游标请求**另一所学校** → 应 `400 CURSOR_SCHOOL_MISMATCH`。4.把游标改成上一版协议生成的字符串（若无旧游标可手工构造 `{"v":1,...}` 的 base64url）→ 应 `409 SCOPE_CHANGED` 并提示重新对账。5.乱造游标字符串 → 应 `400 INVALID_CURSOR`。\n判定：四类非法复用均被明确拒绝且错误码正确 → 通过；任一情况返回 200 数据 → 失败截图（严重）。' },
  ]},
]
