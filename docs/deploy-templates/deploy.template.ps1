# =========================================================
# deploy.template.ps1 — 客户分支部署脚本参考模板
# =========================================================
# 【模板说明】
#   本文件是基于珠海一中分支部署脚本整理的参考模板，存放于 main 分支
#   docs/deploy-templates/ 目录，仅供创建新客户分支部署脚本时参考。
#
#   使用方式：
#     1. 复制本文件到新客户分支，重命名为 deploy.ps1
#     2. 替换所有 [CUSTOMER_NAME] 占位符（共 6 处，搜索替换即可）
#     3. 修改 §1 基础配置中的端口、路径、PM2 名称，确保不与已有系统冲突
#     4. 处理下方所有 # TODO: 标注项
#
#   三系统已知端口占用（截至 2026-07）：
#     RDPMS     → 前端 8080  API 3000  PM2 rdpms-backend
#     田家炳    → 前端 8081  API 3001  PM2 foodtestlab-api
#     珠海一中  → 前端 8082  API 3002  PM2 zhuhaiyizhong-api
#   新客户从 8083/3003 起递增，避免冲突。
#
#   变更记录：见同目录 CHANGELOG.md
# =========================================================
...
$selfRepoRoot  = "C:\[CUSTOMER_NAME]"   # TODO: 替换为客户本地部署根目录
$selfBranch    = "[CUSTOMER_NAME]"      # TODO: 替换为客户对应的 Git 分支名
...
Log "[CUSTOMER_NAME] 食品检验系统一键部署开始"

# =========================================================
# 0. 日志文件
# =========================================================

$logDir = Join-Path $env:TEMP "[CUSTOMER_NAME]-deploy-logs"   # TODO: 替换客户名
...
$deployBranch = if ($env:DEPLOY_BRANCH) { $env:DEPLOY_BRANCH } else { "[CUSTOMER_NAME]" }  # TODO: 替换分支名

$repoRoot     = if ($env:REPO_ROOT)     { $env:REPO_ROOT }     else { "C:\[CUSTOMER_NAME]" }  # TODO: 替换路径
...
$frontendPort = if ($env:FRONTEND_PORT) { [int]$env:FRONTEND_PORT } else { 8083 }  # TODO: 替换为实际前端端口
$apiPort      = if ($env:API_PORT)      { [int]$env:API_PORT }      else { 3003 }  # TODO: 替换为实际 API 端口
$pm2AppName   = if ($env:PM2_APP_NAME)  { $env:PM2_APP_NAME }       else { "[CUSTOMER_NAME]-api" }  # TODO: 替换 PM2 名称
$dataPath     = if ($env:DATA_PATH)     { $env:DATA_PATH }          else { "D:\[CUSTOMER_NAME]-data" }  # TODO: 替换数据目录
...
$zyhFrontendPort   = 8082; $zyhApiPort   = 3002; $zyhPm2Name   = "zhuhaiyizhong-api"
...
$allUsedPorts = @($rdpmsFrontendPort, $rdpmsApiPort, $tjbFrontendPort, $tjbApiPort, $zyhFrontendPort, $zyhApiPort)
if ($allUsedPorts -contains $frontendPort -or $allUsedPorts -contains $apiPort -or $frontendPort -eq $apiPort) {
    Fail "端口冲突：新客户端口必须避开所有已知系统端口。当前 FRONTEND_PORT=$frontendPort, API_PORT=$apiPort"
}

$allUsedPm2Names = @($rdpmsPm2Name, $tjbPm2Name, $zyhPm2Name)
if ($allUsedPm2Names -contains $pm2AppName) {
    Fail "PM2 名称冲突：$pm2AppName 已被其他系统占用"
}

Ok "端口矩阵通过：新客户端口 $frontendPort/$apiPort 未与已知系统冲突"
...
if (Test-Path "prisma") {
    ...
    $dbFile = Join-Path $dataPath "[CUSTOMER_NAME].db"   # TODO: 替换数据库文件名
...
# 持久化 PM2 进程列表（服务器重启后 pm2 resurrect 可恢复）
npx pm2 save
Ok "pm2 save 完成：进程列表已持久化"

# TODO: Windows 开机自启
# Linux 上通过 `pm2 startup` + `pm2 save` 即可实现服务器重启后自动拉起 PM2。
# Windows Server 上没有原生等效命令，有以下两种方案：
#
# 方案 A（推荐，简单）：使用 pm2-windows-startup 包
#   1. 以管理员身份运行 PowerShell
#   2. npm install -g pm2-windows-startup
#   3. pm2-windows-startup install
#   4. 之后每次 pm2 save 即可确保重启后自动恢复
#   注意：pm2-windows-startup 会将 pm2 resurrect 注册为 Windows 登录启动项（Task Scheduler/注册表），
#         需要有服务账号保持登录状态，或改用方案 B。
#
# 方案 B（服务化，无需登录）：使用 nssm 或 winsw 将 PM2 注册为 Windows 服务
#   1. 下载 nssm（https://nssm.cc）
#   2. nssm install pm2-service "node" "C:\...\pm2\bin\pm2 resurrect"
#   3. nssm start pm2-service
#   此方案可做到服务器重启后无需登录即自动启动后端，适合生产环境无人值守场景。
#
# 当前状态：未实现。部署脚本仅执行 pm2 save，服务器重启后需人工执行
#   `npx pm2 resurrect` 或重新运行 deploy.ps1 恢复所有 PM2 进程。
Warn "【TODO】未配置 Windows 开机自启。服务器重启后请手动执行 'npx pm2 resurrect' 或重新运行本脚本。"
...
# TODO: 新增客户时，在下方 $fullNginxConf 的 http{} 块中追加对应的 server{} 块。
#       已有系统的 server{} 块不得修改，仅追加新块。
$fullNginxConf = @"
worker_processes  1;
...
http {
    ...
    # TODO: 新客户 server{} 块在此追加
    # server {
    #     listen [CUSTOMER_FRONTEND_PORT];
    #     server_name _;
    #     root  C:/[CUSTOMER_NAME]/dist;
    #     index index.html;
    #     location / { try_files `$uri `$uri/ /index.html; }
    #     location /api/ {
    #         proxy_pass http://127.0.0.1:[CUSTOMER_API_PORT];
    #         proxy_http_version 1.1;
    #         proxy_set_header Host `$host;
    #         proxy_set_header X-Real-IP `$remote_addr;
    #         proxy_set_header X-Forwarded-For `$proxy_add_x_forwarded_for;
...
Ok "Nginx 多系统配置已写入: $nginxConfPath"
...
Write-Host ""
Write-Host "多系统隔离状态:" -ForegroundColor Cyan
Write-Host "   RDPMS      : 前端 8080  API 3000  PM2 rdpms-backend"
Write-Host "   田家炳     : 前端 8081  API 3001  PM2 foodtestlab-api"
Write-Host "   珠海一中   : 前端 8082  API 3002  PM2 zhuhaiyizhong-api"
Write-Host "   [CUSTOMER] : 前端 $frontendPort  API $apiPort  PM2 $pm2AppName"
