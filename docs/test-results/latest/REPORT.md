...
> 生成时间：2026-08-13 11:00（数据源：数据库 `public."TestResult"`；B7-负向已由开发在服务器侧复核确认为通过）
...
|---|---:|---:|---:|---:|---:|---:|
| 吴翠楠 · 业务功能复测（第一部分~第三部分 + 回归） | 8 | 9 | 0 | 0 | 17/17 | 100% |
| 曾水平 · 备份与恢复模块（第四部分 B1-B9） | 7 | 4 | 0 | 0 | 11/11 | 100% |
| **合计** | **15** | **13** | **0** | **0** | **28/28** | **100%** |
...
| **Q1-果蔬阶段B** Q1 复检显示·果蔬农残（阶段B 刷新） | ❌ 失败 | 吴翠楠 | 2026-08-12 16:07 |
...
- **Q1-果蔬阶段B**（失败）
  - 实际表现：按F5刷新，没有报错，页面直接退出去复检页面
  - 证据图：![Q1 复检显示·果蔬农残（阶段B 刷新）](evidence/Q1-果蔬阶段B/1786522022893_8d6e0942.png)
- **Q1-果蔬阶段C**（失败）
  - 实际表现：没有显示复检合格
...
| **B7-负向** B7 影子恢复·确认词错误不执行 | ✅ 通过 | wuxiangjun | 2026-08-13 11:00 |
...
- **B7-负向**（通过）
  - 前4步网页操作完成；第5步（确认词错误拦截）由开发在服务器侧复核确认：
    - 前端 `backupManager.js` 输错确认词直接 `return`（不发请求）+ 后端 `adminBackupRoutes.js` 兜底 `400` 拒绝
    - 数据库实测：`SystemLog` 无新增 `backup_restore` 记录、无新增旧 schema → 恢复确认未执行，负向拦截成功
