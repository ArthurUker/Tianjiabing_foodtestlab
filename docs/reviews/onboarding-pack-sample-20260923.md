<!-- 本文件为**离线合成的接入包样本**（桩数据，非真实对接方/学校/密钥），用于人工复核导出格式；
     实际交付请在超管界面「开放接口 → 接入说明 → 下载接入包」生成。 -->
# foodSentinel 开放接口 · 接入包

- 对接方：**示例对接方（合成）**
- 契约版本：`v1`
- 生成时间：2026-09-23T04:31:30.722Z
...
- 快照生成时间：2026-09-23T04:31:30.723Z（此后平台仍可调整授权；交付前如已改动请**重新下载**本文件）
...
| `result.result` | 结果文本 | string | — | 否 | 是 | 是 | 如「合格 (<200)」「不合格 (>500)」；实测出现：该类型现有记录均出现（数据观察，非输出保证） |
...
| 类型 | 判定 |
|---|---|
| 餐具 / 果蔬 / 肉蛋 | `result` 文本含「合格」且不含「不合格」→ 合格 |
