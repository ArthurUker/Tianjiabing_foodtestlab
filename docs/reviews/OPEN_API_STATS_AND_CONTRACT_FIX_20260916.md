...
> ⚠️ **后续文档指针（2026-09-17 追加，历史原文一律保留、不改写）**：
> 本报告描述的部分字段与措辞**已被后续工作取代** —— ① `out_of_range_total` **已删除**（语义含糊且会暴露授权范围外记录数），
> 现由 `universe_total` + `request_out_of_range_total` 替代；② TPM 单位措辞由"数值等价于 %/勿再 ×100"改为
> "平台标注 + `unit_verified:false` 待设备协议核实"；③ 统计口径改为"互斥桶 + 恒等式"。
> 最新状态请以 `RDPMS_OPENAPI_P0P1_CORRECTNESS_FIX_20260917.md` 与 `OPEN_API_INTEGRATION.md` 为准。
