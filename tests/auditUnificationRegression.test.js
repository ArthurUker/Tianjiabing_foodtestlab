...
function queryUnifiedAuditTrail({ auditRows, systemRows }, actorId) {
    ...
    const fromSystem = systemRows
        ...
        .map(r => {
            const ctx = r.context ? JSON.parse(r.context) : {};
            return {
                source: 'SystemLog',
...
describe('统一字段规范 v1 · writeSystemLog 自动补齐 canonical 键', () => {
    test('SECURITY:* 事件：actorId→actor_id、userId→target_id、message 前缀→action_type、timestamp→ts，原始键保留', async () => {
        ...
        const ctx = JSON.parse(systemRows[0].context);
...
describe('IF-3/REG-3 · 三类事件经统一 UNION 查询均可检索', () => {
    test('角色变更 + 记录删除 + 安全事件 → queryUnifiedAuditTrail 三条全命中且字段完整', async () => {
        ...
        expect(JSON.parse(byAction['role_change'].details_json)).toMatchObject({ oldRole: 'operator', newRole: 'viewer' });
...
// ====== 4. REG-4 静态回归：server.js DELETE 路由已接线 ======

describe('REG-4 · server.js DELETE /api/admin/schools/:code/users/:userId 静态回归', () => {
    const src = fs.readFileSync(path.resolve(__dirname, '../backend/server.js'), 'utf8');
    const start = src.indexOf("app.delete('/api/admin/schools/:code/users/:userId'");
    const end = src.indexOf('app.', start + 10) > -1 ? src.indexOf('\napp.', start) : src.length;
    ...
    test('server.js 已从统一门面导入 writeAdminOpsLog', () => {
...
describe('第七轮收尾 · BS-11 update_customization 审计经统一门面可被 UNION 查询检索', () => {
    ...
    test('静态回归：customization 路由块调用 writeAdminOpsLog，不再裸写 systemLog.create', () => {
        const src = fs.readFileSync(path.resolve(__dirname, '../backend/server.js'), 'utf8');
        const start = src.indexOf("app.put('/api/admin/schools/:code/customization'");
        expect(start).toBeGreaterThan(-1);
    ...
    test('全文件回归：server.js 不存在任何裸写 prisma.systemLog.create / prisma.auditLog.create', () => {
        const src = fs.readFileSync(path.resolve(__dirname, '../backend/server.js'), 'utf8');
        expect(src).not.toMatch(/prisma\.systemLog\.create/);
        expect(src).not.toMatch(/\.auditLog\.create\(/);
    });
});
