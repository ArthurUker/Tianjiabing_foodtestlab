...
| UI-Version | `login.html` 页脚"系统版本: 1.0" 与 `package.json` `3.1.0` 不一致，需同步 | W7 | 风险修复窗口·2026-07-27 | **已完成**（login.html 改为 3.1.0） | 见最新commit |
...
| TD-Index-Bugs | `index.html` 结构与重复加载问题：① L158-197 `DOMContentLoaded` 两段完全相同的重复绑定；② L64-65 `html2canvas`/`jsPDF` 重复加载（defer 与非 defer 各一次）；③ `btnLogout` id 在 L216 与 L271 重复（违反 HTML 规范）；④ L454 `<select>` 标签缺 `>` 闭合；⑤ L833 `window.tablewareReady` 从未被设置，条件恒 falsy；⑥ L768 formIds 含 `pathogenTestForm` 但 HTML 无此 form id | W7 | 风险修复窗口·2026-07-27 | **已完成**（①⑥ 已修；② 经核实只有一次加载无重复；③ 改用 class `js-logout` 替代重复 id，JS 查询改为 class；④ 经核实无此问题；⑤ 移除恒 falsy 的 tablewareReady 分支） | 见最新commit |
...
| TD-TrustProxy | **高危·限流失效**：server.js 未设 `app.set('trust proxy',...)`，Nginx 反代后 `req.ip` 为代理 IP，rate limit 全局共享单 bucket，单用户可耗尽或绕过。`RATE_LIMIT_MAX_REQUESTS` 默认 1000 偏高 | W3 | 风险修复窗口·2026-07-27 | **已完成**（TST-3：`app.set('trust proxy', 1)` 已加，commit 8443e12） | 见 commit 8443e12 |
| TD-Error-Leak | **高危·信息泄露**：server.js 内联路由 14 处 catch 返回 `details: error.message`（含 SQL 片段/表名/栈），与全局错误处理器（非 dev 隐藏 message）不一致 | W3 | 风险修复窗口·2026-07-27 | **已完成**（server.js + userRoutes + guestRoutes + sessionRoutes + syncRoutes 全部 0 处 error.message 泄露） | 见 commit 3529429 |
...
| TD-XSS-Frontend | **高危·存储型 XSS**：前端多处 `innerHTML` 拼接用户输入未转义：`Pathogen.js:1326`(sampleId/inspector)、`Tableware.js:843,1272`(inspector/correctiveAction)、`UserManagement.js:218`(username/phone)、`BackupRestore.js:671`(err.message)、`Pathogen.js:548`(JSON 嵌 data-* 属性引号转义不足) | W3 | 风险修复窗口·2026-07-27 | **已完成**（Pathogen/UserManagement/GuestDashboard/main.js 已加 escapeHtml；Tableware/BackupRestore 经核实无 innerHTML 注入点） | 见 NB-03 |
...
| TD-Pagination | **中危·DoS**：server.js 多处 `parseInt(limit/offset)` 无上限校验/NaN 防护/负数防护，`?limit=999999999` 一次性查全表 | W5 | 风险修复窗口·2026-07-27 | **已完成**（server.js + auditRoutes + userRoutes 已加 Math.min 500；syncRoutes 经核实无分页参数，不需要修复） | 见 commit 8443e12/3529429 |
...
| TD-GracefulShutdown | **中危·连接泄漏**：`server.js:1051` SIGTERM 的 `server.close()` 无超时，长连接/慢请求致进程挂起，K8s grace period 后 SIGKILL 致 DB 连接未释放 | W5 | 风险修复窗口·2026-07-27 | **已完成**（TST-6：10s forceExit setTimeout 已加，commit 8443e12） | 见 commit 8443e12 |
...
| **TD-RecycleBin-Restored-Filter** | **回收站过滤 `restored`**：`/api/admin/recycle-bin` 端点无 status 过滤，返回所有 `recycle_bin` 行，导致 `status='restored'`（已恢复 = 已重新启用）的学校还显示在回收站列表里；按用户描述"已启用的学校租户不应出现在回收站，只有被停用的才出现"，应只显示 `active`（待恢复）和 `purged`（已彻底删除）。修复：① `backend/routes/schoolRoutes.js:473` SQL 加 `WHERE status IN ('active','purged')`；② `js/modules/adminSchools/views/schoolsListView.js:226` 渲染前 filter 掉 `restored`（双保险 + UI 意图清晰）。**非破坏性**：不影响 restore/purge/archive 接口（仍按 id 查单行） | W当前 | W当前（本窗口·2026-08-18） | 已完成 | 截图2中3条 hqyz"已恢复"记录从回收站列表消失 |
...
| **TD-School-Backup-Sync** | **学校侧备份运维重写**：① 新建 `backend/routes/schoolBackupRoutes.js`（路径 `/api/school/backups`，与超管一致能力但强制本校隔离：`list/run/download/verify/restore` 全按 token 中 `req.user.schoolCode` 过滤，下载/验证/恢复必须先校验 `run.school_code === req.user.schoolCode`，禁止跨校；平台超管 role=admin 且 schoolCode 为空者通过此中间件；审计走 writeTenantAuditLog）；② `server.js` 增加 2 行挂载（§5 公共文件，本窗口先 §2 占行再动）；③ 重写 `js/modules/BackupRestore.js` 为学校侧 cvm 备份视图（删除 localStorage/云端同步旧逻辑，复用 adminSchools/views/backupView.js 的 KPI/列表/分页/立即备份/AES+明文下载/验证/RESTORE 确认模态结构，调 `/api/school/backups`） | W8 | W8（本窗口·2026-08-17） | 已完成 | 新增路由文件不影响已有路由文件；server.js 仅增挂载（沿用现有 auth 中间件风格）；前端整体重写不破坏现有 `BackupRestoreService`+`init/destroy` 对外契约（main.js 调用方式不变）。**2026-08-17 自查修复**：① 后端列表补 `createdBy` 字段（`run_type` 恒为 `scheduled_*`，手动/定时只能以 `created_by` 的 `manual_` 前缀判断，前端 `rowTpl` 同步改用 `createdBy`）；② `download` 端点加 `format ∈ {plain, encrypted}` 白名单（原实现传 `format=xxx` 会绕过明文下载限制落入解密分支，超管版 `adminBackupRoutes.js` 同款缺陷一并修复）；③ 清理 `audit()` 无效 `level` 死参数。**2026-08-18 运维支撑**：① 重启后端 → 新增 `scripts/restart-school-backup-api.sh`（自动探测服务名 + 文件预检 + 健康检查）；② `POST /api/admin/backups/run` 仍 500 —— 非 bug，是 `backupService.js:296` fail-closed 安全机制，新增 `scripts/setup-backup-kms.sh`（自动配置 KMS，模式 A/B 可选，自动备份、幂等写入、可选重启 + dry-run 验证）。**2026-08-18 复盘定位到真正根因**：`deploy.sh` 的 `cat > backend/.env <<EOF` 整体覆盖（白名单 12 变量，不含 `BACKUP_MASTER_KEY`/`TENCENT_*`），且密钥复用循环（395 行）只复用 6 个旧值 → 昨天配置的密钥今天重部署后被冲掉 → `kmsMode()` falsy → 备份 500。**已修复**：`deploy.sh` 复用循环 + 写入块均加入 `BACKUP_MASTER_KEY TENCENT_SECRET_ID TENCENT_SECRET_KEY TENCENT_KMS_REGION TENCENT_KMS_KEY_ID`（首部署空值无害，`kmsMode()` 对空串返回 falsy），`bash -n` 通过；`setup-backup-kms.sh` 警告文案同步更新。**2026-08-18 用户决策：不换新密钥，永久复用昨天保存的 `BACKUP_MASTER_KEY`**——信封加密设计下主密钥可长期复用（每次备份 DEK 独立随机，主密钥只保护 DEK 中间层，非泄露无需轮换），历史 `.aes` 备份可持续恢复。已实测同一密钥加密→解密端到端通过；`setup-backup-kms.sh` 新增 `--key <base64>` 参数（复用旧密钥 + 32 字节长度校验），`--local` 交互模式优先询问粘贴已有密钥、回车才生成新密钥（防误生成导致历史备份不可恢复） |
| **TD-Batch-Restore** | **全库备份批量恢复（大事故应急）**：新增后端 `POST /api/admin/backups/:id/restore-batch`（仅全库备份 scope=all 可批量；body `{confirmText:'RESTORE_ALL', targetSchoolCodes:[...]}`；串行逐校调用 `runRestore`（每校独立原子事务、一校失败不影响其它）；返回逐校结果数组 + 汇总；审计 writeAdminOpsLog action=backup_restore_batch）。前端 `backupView.js` 全库备份行加「批量恢复」按钮 → 弹出学校多选模态（含全选）+ `RESTORE_ALL` 确认词 → 调用 restore-batch → 展示逐校结果。jsdom 完整链路已验证（勾选 s1,s2 → 请求体 codes=s1,s2 confirm=RESTORE_ALL）；后端 5 个非法请求场景 400 校验全通过；lint 0 错误；jest 226/227 通过（唯一失败 authSession.test.js 与本次无关） | W8 | W8（本窗口·2026-08-18） | 已完成 | 路由注册顺序正确（restore-batch 在 restore 之前，Express 精确路径无冲突）；上下文注入依赖 `admin-schools.html:984 initAdminContext`（真实页面已调用）；测试环境需补 initAdminContext 模拟 |
| **TD-Backup-Dir-Migrate** | **备份目录迁移数据盘（容灾隔离）**：核实确认原备份目录 `/var/backups/foodtestlab` 在系统盘 `/dev/vda2`（与 PostgreSQL 同盘，无容灾隔离）；数据盘 `/dev/vdb`（49G，空 47G）挂载 `/mnt/datadisk0`。已完成迁移：`rsync -aHAX` 到 `/mnt/datadisk0/backups`（保留属主权限，980K 数据完整），`.env` `BACKUP_DIR` 改为 `/mnt/datadisk0/backups`，重启后 `backupRootDir()` 确认指向数据盘、服务以 foodtestlab 可写。原目录保留未删（双保险）。**审查修复**：① `restore-batch` 上限 200 校（防误操作/恶意请求拖垮 DB）+ 非字符串元素过滤 + 审计补逐校明细/耗时；② 前端模态显示可选学校总数 + 结果展示优化（成功列数、失败列代码+原因、超20条提示查审计）；③ 服务重启确认 `restore-batch` 端点已装载（401 非 404）、备份目录仍指向数据盘 | W8 | W8（本窗口·2026-08-18） | 已完成 | deploy.sh 重部署不会覆盖 BACKUP_DIR（写入白名单已含）；新备份目录随数据盘独立存活，系统盘故障不丢备份 |
| **TD-Backup-Restore-Extract-Bug** | **影子恢复 `invalid command \` 修复**（截图报错 `school_hqyz` 恢复失败）：根因 `extractSchemaSegment` 用括号配对判断多行语句结束，复杂 CHECK 约束（`ARRAY[...]` 字符串内 + 多层括号）+ PL/pgSQL 函数体（dollar-quote `$_$`）+ 表紧邻 + PG17+ `\restrict` 头全部误判，导致孤立 `\.` 行残留到第 41 行 → psql 报 `invalid command \`。修复改用 pg_dump 标准段锚点（`-- Name:`/`-- Data for Name:`）按段整体取舍，不再做括号配对；同时丢弃 PG17+ psql meta-commands。新增 `tests/extractSchemaSegment.test.js` 7 个用例（含截图 bug 复现），端到端用真实备份 `20260818T105232` 实际跑 psql -f：成功建 16 张表。**注意**：需重启 `foodtestlab-api` 服务才能应用修复 | W8 | W8（本窗口·2026-08-18） | 已完成 | 服务已重启装载新代码；剩余 `authSession.test.js:259` 失败与本任务无关（其它窗口活跃区） |
| **TD-Backup-Restore-PLPGSQL-Parser** | **后续优化·完整 PL/pgSQL 解析器（替代段锚点兜底）**：当前 `extractSchemaSegment` 已用 pg_dump 段锚点修复截图 bug，但**第 5 步兜底路径（restoreSqlUtils.js:163-178）仍保留旧的括号配对算法**——在段锚点未匹配到的异常/非标准 dump 时触发，对 PL/pgSQL 函数体（`$_$` dollar-quote）、字符串内括号、多行注释仍可能误判。**后续工作安排（方案 A）**：为 `extractSchemaSegment` 写一个完整 SQL/PL-pgSQL 词法解析器（识别：单引号/双引号/反引号字符串、`E''` 转义、美元引号 `$tag$`、`--` 行注释、`/* */` 块注释、运算符 `::`），用"语法感知"取代括号配对做语句结束判定；并在 `tests/extractSchemaSegment.test.js` 补齐：嵌套函数体、触发器、`CREATE PROCEDURE`、多行字符串中的分号等边界 case。工作量较大且易引入新 bug，故作为**独立后续任务**（不阻塞当前已修复的恢复功能） | W8 | 后续（排期未定，不建议本周） | 待认领 | 当前段锚点方案对标准 pg_dump 输出 100% 有效（已端到端验证），兜底路径仅影响非标准 dump；Parser 是消除"最后隐患"的锦上添花，优先级低于日常功能 |
