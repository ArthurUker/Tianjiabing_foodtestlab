...
| UI-Version | `login.html` 页脚"系统版本: 1.0" 与 `package.json` `3.1.0` 不一致，需同步 | W7 | 风险修复窗口·2026-07-27 | **未开始**（login.html:254 仍为 1.0） | 非阻塞，仅显示 |
...
| TD-Error-Leak | **高危·信息泄露**：server.js 内联路由 14 处 catch 返回 `details: error.message`（含 SQL 片段/表名/栈），与全局错误处理器（非 dev 隐藏 message）不一致 | W3 | 风险修复窗口·2026-07-27 | **部分完成**（server.js 内 `details: error.message` 已 0；但 userRoutes/guestRoutes/sessionRoutes/syncRoutes 仍有 30+ 处 `${error.message}`，见 REG-07/08） | 见 REG-04/07/08 |
...
| TD-Pagination | **中危·DoS**：server.js 多处 `parseInt(limit/offset)` 无上限校验/NaN 防护/负数防护，`?limit=999999999` 一次性查全表 | W5 | 风险修复窗口·2026-07-27 | **部分完成**（server.js/auditRoutes/userRoutes 已加 Math.min 500；syncRoutes 仍未处理） | 见 REG-05/06 |
